...
class Solution:
    def findLadders(self, beginWord: str, endWord: str, wordList: List[str]) -> List[List[str]]:
        ...
        for i in del_list:
            buckets.pop(i)
        queue = deque([[set(),[beginWord],1]])
        result = []
        final=600
        while queue:
            finish,path,steps=queue.popleft()
            word=path[-1]
            if steps==final:
                break
            for i, _ in enumerate(word):
                ...
                if bucket in finish:
                    continue
                for j in buckets.get(bucket, set()):
                    if j == endWord:
                    ...
                    next_f = finish.copy()
                    next_f.add(bucket)
                    queue.append([next_f, path + [j], steps + 1])
        return result
...
print(Solution.findLadders(None,"cet", "ism", wordList))